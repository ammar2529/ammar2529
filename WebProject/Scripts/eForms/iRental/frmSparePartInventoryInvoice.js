...
AsyncWidgets.WidgetScripts.frmSparePartInventoryInvoice = function (obj) 
{
    ...
    { //
        var strlink = ROOT_PATH + "Pages/eForms/iRental/Reports/PrintInvoice.aspx?FormCode=" + $('[argumentid="InvRecCode"]', t.el).val(); // +'&amp;FormId=' + pm.SelectedKey;
        console.log(strlink);
        var width = 920;
        var height = 600;
    ...
    $('.InvoiceButton_Edit', t.el).click(function () {
        ...
        $('.common-button,.CommonDisableClasss ', t.el).removeClass('');
        ...
        setTimeout(function ()
        {
            ...
            $('[argumentid = "ItemId"], [argumentid = "SelectQuantity"], [argumentid = "SparePartUnitPrice"]', t.el).removeClass('')
    ...
    t.on('show', function (args)
    {
        ...
        if (t.FormMode == "new") 
        {
            ...
            $('.trNoDynamic input',t.el).prop('disabled', true).addClass('');
          ...
          //  $('[argumentid="InvoiceDate"]', t.el).val(dt.getDate() + '/' + (dt.getMonth() + 1) + '/' + dt.getFullYear());
            ...
            $('.StatusRow', t.el).hide();
            $('.btnSaveInvoice', t.el).removeAttr('disabled').removeClass('');
            $('.btnSaveInvoice', t.el).show();

            $('.btnSave,.OnCreateInvoice', t.el).attr('disabled');
            $('.btnSave,.OnCreateInvoice', t.el).addClass('');
    ...
    t.on('afterDataAction', function (params)
    {
        ...
        if (params.res.status === 'OK')
        {
            ...
            $('.btnSaveInvoice', t.el).addClass('');

            $('.btnSave,.OnCreateInvoice', t.el).removeAttr('disabled').removeClass('');
...
AsyncWidgets.WidgetScripts.frmSparePartInventoryInvoice.EnableDisableLineOfItems = function (t)
{
    $('.btnSaveInvoice', t.el).on('click', function ()
    {
        ...
        setTimeout(function ()
        {
            $('.trNoDynamic input').prop('disabled', false).removeClass('');
            $('[argumentid="SparePartSerialNo"],[argumentid="SparePartName"],[argumentid="SparePartQuantity"],[argumentid="TotalPrice"]').prop('disabled', true).addClass('');
...
AsyncWidgets.WidgetScripts.frmSparePartInventoryInvoice.SaveLineOfItem = function (t)
{
    ...
    t.on('onLoadedValues', function (args)
    {
        ...
        $('.SimpleTab', t.el).removeAttr('disabled');
        $('.trNoDynamic input').prop('disabled', false).removeClass('');
        $('[argumentid="SparePartSerialNo"],[argumentid="SparePartName"],[argumentid="SparePartQuantity"],[argumentid="TotalPrice"]',t.el).prop('disabled', true).addClass('');
        $('.StatusRow', t.el).show();
        ...
        if ($('[argumentid="StateId"]', t.el).text() == 'ClosedState')
        {
            ...
            $('.common-button,.CommonDisableClasss,.CommonDisableClass', t.el).addClass('');
        ...
        if ($('[argumentid="StateId"]', t.el).text() == 'CanceledState')
        {
            ...
            $('.common-button,.CommonDisableClasss,.CommonDisableClass,.ItemListDiv', t.el).addClass('');
            $('.InvoiceOpenBtn,.InvoiceButton_Edit', t.el).show();
            $('[argumentid="InvoiceDate" ]', t.el).next('img').hide();
            $('.LineOfItemRow', t.el).hide();
            setTimeout(function () {
