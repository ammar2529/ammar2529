...
function ($) {
    $.fn.timeout = function (options) {
        ...
        var defaults = {
            timeoutInSeconds: 1140,// 1140, //60, //1140,         
    ...
    function showToolbar(timer, opts) {
        ...
        if (opts.timeoutDebug)
        {
            console.log("Session Timeout JS Firing Toolbar Display");
        }
        clearTimeout(timer);
        timer = null;
        ...
        $("#sessionTimeoutToolbar")
            .stop()
            .animate({ "marginTop": (win.scrollTop()) + "px" }, "slow");

        win.bind('scroll.sessionbar',
            function ()
            {
                $("#sessionTimeoutToolbar").stop().animate({ "marginTop": (win.scrollTop()) + "px" }, "slow");
            }
        );
        var toCall = function () {
            warningCountdown(timer, opts, opts.timeoutWarningInSeconds);
        };
        timer = setTimeout(toCall, 1000);
        $('#sessionTimeoutToolbar').click(function ()
        {
            fireKeepAlive(timer, opts);
            $(this).remove();
            $(window).unbind('scroll.sessionbar');
        });
        $('#sessionTimeoutToolbar').hover(function ()
        {
    ...
    function warningCountdown(timer, opts, currentSecond) {
        clearTimeout(timer);
        timer = null;
        if ($('#sessionTimeoutToolbar').length == 0)
        {
            return;
        }
        $('#sessionTimeoutToolbarCountdown').html(currentSecond + " Second(s) Remain");
        if (currentSecond < 1)
        {
            timedOut(timer, opts);
            return;
        ...
        var toCall = function ()
        {
            warningCountdown(timer, opts, (currentSecond - 1));
        };
        timer = setTimeout(toCall, 1000);
    ...
    function timedOut(timer, opts) {
        ...
        window.location = opts.timeoutURL;
        return;
    }
    function fireKeepAlive(timer, opts) {
        ...
        $.ajax({
            url: opts.SessionAlive, cache: false
        });
    ...
    function initialize(timer, opts, firstInit) {
        if (window.location.href.indexOf('.htm') == -1) {
            ...
            var totalTicks = opts.timeoutInSeconds * 1000;
            var warningTicks = opts.timeoutWarningInSeconds * 1000;
            var toCall = function () {
            ...
            AsyncWidgets.user.on('actionPerformed', function () {
                ...
                timer = null;
                timer = setTimeout(toCall, (totalTicks - warningTicks));
                console.log("actionPer:" + (totalTicks - warningTicks));
            });
            // End
        }
        return;
